...
function trackProceedToCheckout(){
	if(typeof trackEvent == "function"){
		  ...
		  trackEvent(analyticsAppName+'_ADDTOCART',analyticsAppName+'_PROCEED_TO_CHECKOUT');//Webtrends tracking
	}
...
var ibpMiniCart = {
		...
		mc_bindMiniCartEvents : function () {
			...
			jQuery('.mc-remove-item-link').click(function(){
				...
				var midHeight = jQuery(itemrow).height();
				if(midHeight < 50 ){
					midHeight = 50;
		        }
				
		        var middleHeight = '50px';
				...
				jQuery('#mc-proceed-checkout-button').attr('disabled','disabled').attr('src','/images/button-proceed-checkout-dis.gif');
			...
			jQuery('.mc-cancel-remove-item').click(function(){
				...
				jQuery('#mc-proceed-checkout-button').attr('disabled','').attr('src','/images/button-proceed-checkout.gif');
		    ...
		    jQuery('#mc-proceed-checkout-button').click(function(evt){
		    	if(typeof refSite != 'undefined' && typeof refSiteName != 'undefined'){
		    		MC_MINICART_CHECKOUT_URL += "?refSite="+refSite + "&refSiteName="+refSiteName; 
		    	}
		    	...
		    	setTimeout(function() {
		    		window.location.href = MC_MINICART_CHECKOUT_URL;
		    	}, millisecondsToWait);
			...
			if ((miniCart.itemGroups != null && miniCart.itemGroups.length != 0)|| (miniCart.donations != null && miniCart.donations.length != 0)) {
				...
				jQuery.each(miniCart.itemGroups, function(){
					...
					jQuery.each(itemGroup.items, function(){
						...
						if(assetType != null && assetType != "undefined" && assetType == "Monthly"){
						...
						if(mediaType != null && mediaType != "undefined" && mediaType != "" && mediaType != "All"){
						...
						if(prodDetailUrl != undefined && prodDetailUrl != null && prodDetailUrl != "" && prodDetailUrl != "undefined"){
				...
				jQuery(".mc-content").append(
					...
					'	<p class="mc-disclaimer mc-checkout">Sales tax, if applicable, and shipping charges are calculated during Checkout</p>\n' +
					'	<p class="text-right">\n' +
				...
				if (miniCart.hasMembershipProduct && !miniCart.membershipComplete) {
			        ...
			        '<p class="confirmation-description">For membership and subscriptions, you will need to <a href="'+ MC_COMPLETE_MEMBERSHIP_URL +'">complete an application </a>first before proceeding with checkout.</p>'+
			        '</div>');
				}
		...
		mc_processAddItem : function(items, mc_success, mc_error){
			...
			jQuery.ajax({
				  ...
				  error: function(request, text, errorThrown){
					  ...
					  if(typeof trackEvent == "function"){
		    			  var analyticsAppName = mc_getAnalyticsAppName();
		    			  trackEvent(analyticsAppName+'_ADDTOCART',analyticsAppName+'_ADDTOCART_FAILED');//Webtrends tracking for add cart success
		    		  }
				  }
			   }
			);
		...
		mc_addItemQueue : function(qItems, mc_success, mc_error) {
		  ...
		  if(typeof trackEvent == "function"){
			  var analyticsAppName = mc_getAnalyticsAppName();
			  trackEvent(analyticsAppName+'_ADDTOCART',analyticsAppName+'_ADDTOCART');//Webtrends tracking for add cart
		  }
		  ...
		  complete: function(){
			 ...
			 setTimeout(function() {jQuery('#mc-items-added').toggle('fast');}, 5000);
			  ...
			  if(typeof trackEvent == "function"){
				  var analyticsAppName = mc_getAnalyticsAppName();
				  trackEvent(analyticsAppName+'_ADDTOCART',analyticsAppName+'_ADDTOCART_SUCCESS');//Webtrends tracking for add cart success
			  }
		  }
		  });
